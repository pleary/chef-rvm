      ...
      class RVMRubygems < Chef::Provider::Package::Rubygems
        ...
        def install_package(name, version)
          ...
          ruby_strings.each do |rubie|
            ...
            e = ::Chef::Resource.resource_for_node(:rvm_environment, node).new(rubie, @run_context)
        ...
        def install_via_gem_command(name, version)
          ...
          if node['rvm']['default_ruby'].match( /ruby-2\.[6-9]/ )
            cmd << %{ install #{name} -q --no-document -v "#{version}" --force}
          else
